...
void Organism::look_for_new_promoters_starting_between(int32_t pos_1,int32_t pos_2) {
	...
	#pragma omp parallel 
	#pragma omp for 
    for (int32_t i = pos_1; i < pos_2; i++) {			//PARALLEL appel � promoter_at
        ...
        if (dist <= 4) {
			...
			/*	cout << "promidx =  ";
               ...
               // count_prom = count_prom + 1;
				#pragma omp critical							//TODO am�liorer le grain en utilisant ATOMIC
				{
...
void Organism::look_for_new_promoters_starting_after(int32_t pos) {
	...
	#pragma omp parallel
	#pragma omp for 
    for (int32_t i = pos; i < dna_->length(); i++) {			//PARALLEL appel � promoter_at
